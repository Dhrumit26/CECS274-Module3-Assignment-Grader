...
try:
    ...
    def TestCase():
      ...
      try:
        student_stack.pop()
        msg = "Created empty SLLStack and attempted to remove an element.\nResult: IndexError was not raised.\nTest failed."
        return TestOutput(passed=False, logs=msg)
      except IndexError:
        return TestOutput(passed=True, logs="Created empty SLLStack and attempted to remove an element.\nResult: IndexError is correctly raised. \nTest passed.")
      except Exception as e:
        return TestOutput(passed=False, logs=f"Created empty SLLStack and attempted to remove an element.\n\nThe following unexpected error occurred: {e}")
...
f = open("/outputs/146278.json", "w")
json.dump({"id": "146278", "passed": output.passed, "log": output.logs}, f)
f.close()
